...
def getVar(scope, var_name):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        var = tf.get_variable(var_name)
    return var
conv1_historgram = tf.summary.histogram("conv1_weights", getVar("conv1", "weights"))
conv2_historgram = tf.summary.histogram("conv2_weights", getVar("conv2", "weights"))
fc4_historgram = tf.summary.histogram("fc4_weights", getVar("fc4", "weights"))
fc3_historgram = tf.summary.histogram("fc3_weights", getVar("fc3", "weights"))
...
with tf.Session(config=config) as sess:
    ...
    writer_1 = tf.summary.FileWriter("./logs/18/tr", graph=tf.get_default_graph())
    writer_2 = tf.summary.FileWriter("./logs/18/vl")
