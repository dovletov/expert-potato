...
with tf.Session(config=config) as sess:
    ...
    writer_1 = tf.summary.FileWriter("./logs/20/tr", graph=tf.get_default_graph())
    writer_2 = tf.summary.FileWriter("./logs/20/vl")
    ...
    orig_img = mnist.test.images[0:50,:].reshape((-1, 28, 28, 1))
    print(orig_img.shape)
    recon_img = sess.run(y, feed_dict=feed_test)[:50]
    ...
    for i in range(50):
        plt.subplot(5, 10, i+1)
        plt.imshow(orig_img[i, ..., 0], cmap='gray')
    plt.show()
